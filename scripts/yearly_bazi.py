...
BANHE = {"卯未": "亥卯未木局", "午未": None, "亥卯": "亥卯未木局", "亥未": "亥卯未木局"}
...
def zhi_rel(z1: str, z2: str) -> str:
    ...
    if pair in BANHE:
        return "半合"
    if pair in {"丑戌", "戌丑", "戌未", "未戌", "丑未"}:
        return "刑" if pair == "丑未" else "刑"
    return ""
...
def suiyun_rel(year_gz: str, luck_gz: str) -> list[str]:
    ...
    if "".join(sorted(yg + lg)) in LIUHE:
        tags.append("岁运干合")
    if "".join(sorted((yz, lz))) in LIUCHONG:
        tags.append("岁运支冲")
    if "".join(sorted((yz, lz))) in LIUHE:
        tags.append("岁运支合")
    return tags
...
def main():
    ...
    ap.add_argument("--score", help="事实表 JSON（打分模式）")
